...
Ce guide explique comment créer un nouveau package dans le monorepo et le publier sur npm sous le scope `@lec-core`.
...
- **`name`** : Doit commencer par `@lec-core/`
...
Le monorepo fournit une config TypeScript partagée via `@lec-core/typescript-config`. Le fichier `base.json` active le mode strict et les options standards.
...
La config partagée `@lec-core/tsup-config` génère automatiquement :
...
Si ton package dépend d'un autre package du monorepo (par exemple `@lec-core/ddd-tools`), ajoute-le dans les `dependencies` :
...
1. L'organisation `@lec-core` existe sur [npmjs.com](https://www.npmjs.com/org/lec-core)
...
- [ ] Le `name` suit la convention `@lec-core/nom-du-package`
- [ ] Le `package.json` contient `publishConfig.access: "public"`
- [ ] Le `tsconfig.json` extend `@lec-core/typescript-config/base.json`
- [ ] Le `tsup.config.ts` utilise `@lec-core/tsup-config`
- [ ] Le `src/index.ts` exporte tout ce qui est public
- [ ] Le build passe (`yarn build --filter=@lec-core/nom`)
- [ ] Le lint passe (`yarn lint --filter=@lec-core/nom`)
